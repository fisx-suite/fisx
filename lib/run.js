...
function init(fis, argv, env) {
    ...
    // listen error
    fis.on('release:error', function (err) {
        fis.log.warn('release error: %s', (err.stack || err.toString()).red);
    });
}

function loadProjectConfig(fis, argv, env) {
    var cmdName = argv._[0];
    var needInitConfig = cmdName === 'release';
    ...
    // 如果启用 watch livereload，关闭页面文件的缓存
    if (argv.w && argv.L) {
        fis.match('*.{html,tpl}', {
            useCache: false
        });
...
function run(fis, argv, env) {
    ...
    try {
        ...
        fis.log.info('unknown command, please use `fisx --help` to show all available commands');
        return;
    }
